    ...
    void listen_queues(void)
    {
        ...
        if (strcmp(broker, "RabbitMQ") == 0)
        {
            ...
            if (resp.reply_type != AMQP_RESPONSE_NORMAL)
                utils::logger(ERROR, "Cannot login to RabbitMQ", "", 0);
            amqp_channel_open(rconn, 1);
            resp = amqp_get_rpc_reply(rconn);
            if (resp.reply_type != AMQP_RESPONSE_NORMAL)
                utils::logger(ERROR, "Cannot open RabbitMQ channel", "", 0);
            amqp_basic_consume(rconn, 1, amqp_cstring_bytes(broker_queue), amqp_empty_bytes,
                            0, 0, 0, amqp_empty_table);
            resp = amqp_get_rpc_reply(rconn);
            if (resp.reply_type != AMQP_RESPONSE_NORMAL)
                utils::logger(ERROR, "Cannot get reply from RabbitMQ", "", 0);
        ...
        else if (strcmp(broker, "ApacheKafka") == 0)
        {
                                ...
                                errstr, sizeof(errstr)) != RD_KAFKA_CONF_OK)
            {
                ...
                utils::logger(ERROR, "Cannot login to Kafka", errstr, 0);
            }
            if (rd_kafka_conf_set(conf, "group.id", groupid,
                                errstr, sizeof(errstr)) != RD_KAFKA_CONF_OK)
            {
                rd_kafka_conf_destroy(conf);
                utils::logger(ERROR, "Cannot set Kafka group id", errstr, 0);
            }
            if (rd_kafka_conf_set(conf, "auto.offset.reset", "earliest",
                                errstr, sizeof(errstr)) != RD_KAFKA_CONF_OK)
            {
                rd_kafka_conf_destroy(conf);
                utils::logger(ERROR, "Cannot set Kafka auto.offset.reset", errstr, 0);
            }
            rk = rd_kafka_new(RD_KAFKA_CONSUMER, conf, errstr, sizeof(errstr));
            if (!rk)
                utils::logger(ERROR, "Cannot create Kafka consumer instance", errstr, 0);
            ...
            if (err)
            {
                ...
                rd_kafka_destroy(rk);
                utils::logger(ERROR, "Cannot subscribe to Kafka queues", (char *)rd_kafka_err2str(err), 0);
            }
            ...
            if (rd_kafka_conf_set(oconf, "bootstrap.servers", brokers, errstr, sizeof(errstr)) != RD_KAFKA_CONF_OK)
                utils::logger(ERROR, "Cannot connect to outgoing Kafka ", errstr, 0);
            rd_kafka_conf_set_dr_msg_cb(oconf, dr_msg_cb);
            ork = rd_kafka_new(RD_KAFKA_PRODUCER, oconf, errstr, sizeof(errstr));
            if (!ork)
                utils::logger(ERROR, "Failed to create Kafka producer", errstr, 0);
        }
        else return;
        ...
        utils::logger(LOG, "mini-3 queues listener starting", "", getpid());
        while (true)
        {
            if (strcmp(broker, "RabbitMQ") == 0)
            {
                ...
                {
                    utils::logger(ERROR, "Abnormal RabbitMQ reply", "", 0);
                    break;
                }
                char *req = (char *)malloc(envelope.message.body.len + 1);
                if(!req) utils::out_of_memory();
                memcpy(req, envelope.message.body.bytes, envelope.message.body.len);
                req[envelope.message.body.len] = 0;
                //utils::logger(LOG, "Queue process\t", "Going to process request\n", 0);
                char *answer = utils::process_request(req, 0, 0, 0, global_web_pip);
                //utils::logger(LOG, "Queue process\t", "Returned from process request\n", 0);
                ...
                if (answer)
                {
                    ...
                    props.delivery_mode = 2; // persistent delivery mode
                                            //utils::logger(LOG, broker_output_queue, answer, 0);
                    ...
                    if (code < 0)
                    {
                        ...
                        utils::logger(LOG, "Error", answer, 0);
                    }
                    free(answer);
            ...
            else if (strcmp(broker, "ApacheKafka") == 0)
            {
                ...
                if (rkm->payload)
                {
                    ...
                    if(!rq) utils::out_of_memory();
                    memcpy(rq, rkm->payload, rkm->len);
                    rq[rkm->len] = 0;
                    char *answer = utils::process_request(rq, 0, 0, 0, global_web_pip);
                ...
                retry:
                    ...
                    if (err)
                    {
                        utils::logger(LOG, "Failed to produce to Kafka", (char *)rd_kafka_err2str(err), 0);
