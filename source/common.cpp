void logger(int type, char *s1, char *s2, int socket_fd) {
	...
	switch (type) {
        case ERROR: (void)sprintf(logbuffer,"503 ERROR: %s: %s Errno=%d exiting pid=%d",s1, s2, errno, getpid()); 
            break;
        case FORBIDDEN: 
        ...
        case NOTFOUND: 
            ...
            break;
        case LOG: (void)sprintf(logbuffer," %s ",s1);
            if(s2[0] != 0) strcat(logbuffer, s2);
            if(socket_fd != 0) sprintf((char*)((long)logbuffer+strlen(logbuffer)), " = %i", socket_fd);
	}	
    #ifndef WEBSERVER
    printf("%s\n",logbuffer);
    #endif
	...
	sem_post(lsem);
	free(logbuffer);
	if(type == ERROR || type == NOTFOUND || type == FORBIDDEN) exit(0);
}
...
void unicode_to_utf8(char *buffer) {
    ...
    for(int i=0; i<r; i++) {
        if(buffer[i]=='\\' && buffer[i+1]=='u') {
            char ch1[5], ch2[5]; int cd1, cd2, cd;
            ch1[0] = '0'; ch1[1] = 'x'; ch1[2] = buffer[i+2]; ch1[3] = buffer[i+3]; ch1[4] = 0;
            ch2[0] = '0'; ch2[1] = 'x'; ch2[2] = buffer[i+4]; ch2[3] = buffer[i+5]; ch2[4] = 0;
            sscanf(ch1, "%x", &cd1);
            sscanf(ch2, "%x", &cd2);
            cd = (cd1 << 8) + cd2;
            if(cd<0x80) t[tp++] = (char)cd;
            else if(cd<0x800) { t[tp++] = 192+cd/64; t[tp++] = 128+cd%64; }
            else if(cd<0x10000) { t[tp++] = 224+cd/4096; t[tp++] = 128+cd/64%64; t[tp++] = 128+cd%64; }
            else if(cd<0x110000) { t[tp++] = 240+cd/262144; t[tp++] = 128+cd/4096%64; t[tp++] = 128+cd/64%64; t[tp++] = 128+cd%64; }
            i += 5;
        }
        else
            t[tp++] = buffer[i];
