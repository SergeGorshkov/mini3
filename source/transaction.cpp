...
bool write_to_pipe(int pip, int command, local_triple *t) {
    ...
    do {
        ...
        if(PIPESIZE - nbytes > 8192 + strlen(t->o)) break;
        usleep(1000);
    } while(true);
    if(!write(pip, &command, sizeof(int))) return false;
    if(!write(pip, t, sizeof(indexable))) return false;
    int a = t->s ? strlen(t->s) : 0;
    if(!write(pip, &a, sizeof(int))) return false;
    if(a) { if(!write(pip, t->s, a)) return false; }
    a = t->p ? strlen(t->p) : 0;
    if(!write(pip, &a, sizeof(int))) return false;
    if(a) { if(!write(pip, t->p, a)) return false; }
    a = t->o ? strlen(t->o) : 0;
    if(!write(pip, &a, sizeof(int))) return false;
    if(a) { if(!write(pip, t->o, a)) return false; }
    a = t->d ? strlen(t->d) : 0;
    if(!write(pip, &a, sizeof(int))) return false;
    if(a) { if(!write(pip, t->d, a)) return false; }
    if(!write(pip, t->l, 8)) return false;
    return true;
}
...
void read_from_pipe(int pip, local_triple *t) {
    ...
    sem_wait(rsem);
	int a = 0;
	if(!read(pip, t, sizeof(indexable))) { sem_post(rsem); return; }
...
unsigned long insert_into_index(char *s, mini_index *index, unsigned long *f_chunks_size) {
    ...
    if(f_chunks_size[target_chunk] == CHUNK_SIZE) {
        ...
        if(!found) for(unsigned long i = target_chunk - 1; i >= 0; i--) {
            if(f_chunks_size[i] < CHUNK_SIZE) {
                target_chunk = i;
                pos = f_chunks_size[i];
                found = true;
...
char *global_delete_triple(unsigned long pos) {
    char message[1024];
    if(pos < 0 || pos >= (*n_triples)) return NULL;
    sem_wait(sem);
